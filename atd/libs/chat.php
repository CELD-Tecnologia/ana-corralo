...
function post_message_($threadid, $kind, $message, $link, $from = null, $utime = null, $opid = null)
{
	...
	$query = sprintf(
		...
		$threadid,
		$kind,
		mysqli_real_escape_string($message, $link),
		$from ? "'" . mysqli_real_escape_string($from, $link) . "'" : "null",
		$opid ? $opid : "0",
...
function rename_user($thread, $newname)
{
	...
	$link = connect();
	commit_thread($thread['threadid'], array('userName' => "'" . mysqli_real_escape_string($newname, $link) . "'"), $link);
...
{
	global $mysqlprefix;
	return select_one_row("select banid,comment from ${mysqlprefix}chatban where unix_timestamp(dtmtill) > unix_timestamp(CURRENT_TIMESTAMP) AND address = '" . mysqli_real_escape_string($addr, $link) . "'", $link);
}
...
function create_thread($groupid, $username, $remoteHost, $referer, $lang, $userid, $userbrowser, $initialState, $link)
{
	...
	$query = sprintf(
		...
		mysqli_real_escape_string($username, $link),
		mysqli_real_escape_string($userid, $link),
		next_token(),
		mysqli_real_escape_string($remoteHost, $link),
		mysqli_real_escape_string($referer, $link),
		next_revision($link),
		mysqli_real_escape_string($lang, $link),
		mysqli_real_escape_string($userbrowser, $link));
...
function do_take_thread($threadid, $operatorId, $operatorName)
{
	...
	commit_thread($threadid,
				  array("istate" => $state_chatting,
					   "nextagent" => 0,
					   "agentId" => $operatorId,
					   "agentName" => "'" . mysqli_real_escape_string($operatorName, $link) . "'"), $link);
	mysqli_close($link);
}
...
function check_connections_from_remote($remote, $link)
{
	...
	$result = select_one_row(
		...
		"where remote = '" . mysqli_real_escape_string($remote, $link) . "' AND istate <> $state_closed AND istate <> $state_left", $link);
