...
function connect()
{
	...
	mysqli_select_db($mysqldb, $link) or die('Could not select database');
	if ($force_charset_in_connection) {
		mysqli_query("SET NAMES '$dbencoding'", $link);
	}
	return $link;
}
...
{
	mysqli_query($query, $link) or die(' Query failed: ' . mysqli_error($link));
}

function select_one_row($query, $link)
{
	$result = mysqli_query($query, $link) or die(' Query failed: ' . mysqli_error($link));
...
{
	$sqlresult = mysqli_query($query, $link) or die(' Query failed: ' . mysqli_error($link));
...
{
	$result = mysqli_query(db_build_select("count(" . ($countfields ? $countfields : "*") . ")", $table, $conditions, ""), $link)
	or die(' Count query failed: ' . mysqli_error($link));
...
function loadsettings_($link)
{
	...
	$sqlresult = mysqli_query("select vckey,vcvalue from ${mysqlprefix}chatconfig", $link) or die(' Query failed: ' . mysqli_error($link));
