...
function operator_by_login($login)
{
	...
	$operator = select_one_row(
		"select * from ${mysqlprefix}chatoperator where vclogin = '" . mysqli_real_escape_string($login) . "'", $link);
	mysqli_close($link);
	return $operator;
}
...
function operator_by_email($mail)
{
	...
	$link = connect();
	$operator = select_one_row(
		"select * from ${mysqlprefix}chatoperator where vcemail = '" . mysqli_real_escape_string($mail) . "'", $link);
	mysqli_close($link);
	return $operator;
}
...
function update_operator($operatorid, $login, $email, $password, $localename, $commonname)
{
	...
	$query = sprintf(
		...
		mysqli_real_escape_string($localename),
		mysqli_real_escape_string($commonname),
...
function update_operator_avatar($operatorid, $avatar)
{
	...
	$query = sprintf(
		...
		mysqli_real_escape_string($avatar), $operatorid);
...
function create_operator_($login, $email, $password, $localename, $commonname, $avatar, $link)
{
	...
	$query = sprintf(
		...
		mysqli_real_escape_string($localename),
		mysqli_real_escape_string($commonname),
		mysqli_real_escape_string($avatar),
		mysqli_real_escape_string($email), '');
