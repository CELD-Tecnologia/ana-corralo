...
function group_by_name($name)
{
	...
	$group = select_one_row(
		"select * from ${mysqlprefix}chatgroup where vclocalname = '" . mysqli_real_escape_string($name) . "'", $link);
	mysqli_close($link);
	return $group;
}
...
function create_group($name, $descr, $commonname, $commondescr, $email)
{
	...
	$query = sprintf(
		...
		mysqli_real_escape_string($descr),
		mysqli_real_escape_string($commonname),
		mysqli_real_escape_string($commondescr),
