...
{
	$res = mysqli_query($query, $link) or show_install_err(' Query failed: ' . mysqli_error());
	return $res;
}

$act = verifyparam("act", "/^(silentcreateall|createdb|ct|dt|addcolumns)$/");

$link = @mysqli_connect($mysqlhost, $mysqllogin, $mysqlpass)
		 or show_install_err('Could not connect: ' . mysqli_error($link));

if ($act == "silentcreateall") {
	mysqli_query("CREATE DATABASE $mysqldb", $link) or show_install_err(' Query failed: ' . mysqli_error($link));
	...
	mysqli_select_db($mysqldb, $link) or show_install_err('Could not select database');
	if ($force_charset_in_connection) {
		mysqli_query("SET character set $dbencoding", $link);
	}
	...
	if ($act == "ct") {
		...
		foreach (array_keys($dbtables) as $id) {
			mysqli_query("DROP TABLE IF EXISTS $id", $link) or show_install_err(' Query failed: ' . mysqli_error($link));
		}
		...
		$res = mysqli_query("select null from information_schema.statistics where table_name = '${mysqlprefix}chatmessage' and index_name = 'idx_agentid'", $link);
