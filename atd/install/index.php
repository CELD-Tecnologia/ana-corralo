...
function check_connection()
{
	...
	if ($link) {
		$result = mysqli_query("SELECT VERSION() as c", $link);
		if ($result && $ver = mysqli_fetch_array($result, mysqli_ASSOC)) {
			$page['done'][] = getlocal2("install.1.connected", array($ver['c']));
			mysqli_free_result($result);
		} else {
			$errors[] = "Version of your SQL server is unknown. Please check. Error: " . mysqli_error($link);
			mysqli_close($link);
			return null;
		}
		return $link;
	} else {
		$errors[] = getlocal2("install.connection.error", array(mysqli_error()));
		return null;
	}
}
...
function check_database($link)
{
	...
	if (mysqli_select_db($mysqldb, $link)) {
...
{
	global $mysqlprefix;
	$result = mysqli_query("select * from ${mysqlprefix}chatoperator where vclogin = 'admin'", $link);
	if ($result) {
